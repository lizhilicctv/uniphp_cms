	...
	<form action="" method="post" class="form form-horizontal" id="form-admin-role-add">
		<div class="row cl">
			...
			<div class="formControls col-xs-9 col-sm-10">
				<input type="text" class="input-text" value="" placeholder="" id="title" name="title" datatype="*4-16" nullmsg="用户账户不能为空">
			</div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-9 col-sm-10">
				...
				<?php foreach($this->datasort as $k=>$v){ ?>
					<?php if($v['level'] == 0){ 
						...
						<dt>
							<label>
								<input type="checkbox" value="<?php echo $v['id']; ?>"  id="f<?php echo $v['id']; ?>" name="rules[]">
								<?php echo $v['title']; ?></label>
						</dt>
					<?php }else{ ?>
							<div class="check-box" value="<?php echo $v['id']; ?>"  style="padding: 8px;">
								<label ><input type="checkbox" class="f<?php echo $v['id']; ?>"  name="rules[]" value="<?php echo $v['id']; ?>">
							   <?php echo $v['title']; ?></label>
							  </div>
					<?php } ?>
