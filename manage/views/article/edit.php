	...
	<form class="form form-horizontal" enctype="multipart/form-data"  action="" method="post" id="form-article-add">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-9 col-sm-9">
				<input type="text" class="input-text" value="<?php echo $this->data['title']; ?>" placeholder="请输入文章标题" id="title" name="title">
			</div>
		</div>
			...
			<div class="formControls col-xs-9 col-sm-9"> <span class="select-box">
				<select  name="cateid" class="select">
					...
					<?php foreach($this->datasort as $k=>$v){ ?>
						<option <?php if($v['id']==$this->data['cateid']){echo 'selected';} ?> value="<?php echo $v['id']; ?>"><?php if(isset($v['level'])){echo str_repeat("|--",$v['level']);} echo $v['catename']; ?></option>
					<?php } ?>
				</select>
				</span> </div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-9 col-sm-9">
				<input type="text" class="input-text" value="<?php echo $this->data['keyword']; ?>" placeholder="请输入关键词" id="keyword" name="keyword">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-3 col-sm-2">文章摘要：</label>
			<div class="formControls col-xs-9 col-sm-9">
				<textarea name="desc" id="desc" rows="" class="textarea"  placeholder="说点什么...最少输入10个字符" datatype="*10-100" dragonfly="true" nullmsg="备注不能为空！" onKeyUp="textlength()"><?php echo $this->data['desc']; ?></textarea>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-9 col-sm-9">
				<input type="text" class="input-text" value="<?php echo $this->data['author']; ?>" placeholder="" id="author" name="author">
			</div>
		</div>
		<div class="row cl">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-9 col-sm-9"> 
				<script id="editor" name='editor' type="text/plain" style="width:100%;height:400px;"><?php echo html_entity_decode($this->data['editor']); ?></script> 
			</div>
		</div>
		<div class="row cl">
